...
function App() {
  ...
  const writeChar = async (nodeName, char) =>
    new Promise((resolve) => {
      setTimeout(() => {
        if (nodeName === "page-wrapper") {
          setStyletext((text) => {
            ...
            let html = (char = "*"
              ? Prism.highlight(temp, Prism.languages.css, "css")
              : temp);
  ...
  const writeTo = async (nodeName, index, text) => {
  ...
  return (
    <>
      <div className="page-wrapper" ref={text}>
        ...
        <style
          dangerouslySetInnerHTML={{ __html: styleText.styleText }}
        ></style>
      </div>
      <div
